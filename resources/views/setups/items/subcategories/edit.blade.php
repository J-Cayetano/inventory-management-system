        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-header">
                    <h3 class="card-title">Update {{ str_replace('ie', 'y', substr(ucwords($key), 0, -1)) }}: {{ $subcategory->name }}</h3>
                </div>
                {{-- Body --}}
                <div class="card-body">
                    <form method="POST" action="{{ route($key . '.update', $subcategory->id) }}" enctype="multipart/form-data"
                        id="create-form">
                        @csrf
                        @method('PUT')
                        <div class="row g-2">
                            <div class="col-sm-3 p-5">
                                <div class="form-group">
                                    ...
                                    <input class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}"
                                        ...
                                        value="{{ old('code', $subcategory->code) }}" required>
                                    @error('code')
                        ...
                        <div class="row g-2">
                            <div class="col-sm-3 p-5">
                                <div class="form-group">
                                    <label class="required form-label"
                                        for="category_id">{{ __('cruds.' . $key . '.fields.category_id') }}</label>
                                    <select class="form-select {{ $errors->has('category_id') ? 'is-invalid' : '' }}"
                                        name="category_id" id="category_id" value="{{ old('category_id', '') }}" required>
                                        <option selected disabled>Select a Category</option>
                                        @foreach ($categories as $category)
                                            <option value="{{ $category->code }}"
                                                {{ $category->code === $subcategory->category->code ? 'selected' : '' }}>
                                    ...
                                    @enderror
                                    <span
                                        class="help-block text-info fs-5">{{ __('cruds.' . $key . '.fields.category_id_helper') }}</span>
                                </div>
