        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-header">
                    <h3 class="card-title">Update {{ substr(ucwords($key), 0, -1) }}: {{ $brand->name }}</h3>
                </div>
                {{-- Body --}}
                <div class="card-body">
                    <form method="POST" action="{{ route($key . '.update', $brand->id) }}" enctype="multipart/form-data"
                        id="create-form">
                        @csrf
                        @method('PUT')
                        <div class="row g-2">
                            <div class="col-sm-3 p-5">
                                <div class="form-group">
                                    ...
                                    <input class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}"
                                        type="text" name="code" id="code" value="{{ old('code', $brand->code) }}"
                                        required>
                                    @error('code')
